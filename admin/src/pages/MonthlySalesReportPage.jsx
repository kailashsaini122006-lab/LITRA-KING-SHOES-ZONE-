...
export default function MonthlySalesReportPage({ accessToken }) {
  ...
  const fetchMonthlyReport = useCallback(async (monthStr) => {
    ...
    try {
      ...
      const url = getApiUrl(`/orders/reports/monthly?monthStr=${encodeURIComponent(targetMonth)}`);
      ...
      if (res.ok && data && data.success) {
        ...
        setError(data?.message || `Failed to fetch monthly sales report (${res.status}).`);
      }
    } catch (err) {
  ...
  const formatMonthLabel = (monthStr) => {
    ...
    return dateObj.toLocaleDateString('en-IN', { year: 'numeric', month: 'long' });
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4 sm:p-6 lg:p-8 pt-6 sm:pt-8">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-400">
                Complete monthly revenue collection, payment method split, and order analytics.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 rounded-xl text-xs text-emerald-300 font-semibold">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-emerald-400" />
            <span>Selected Month: <strong className="text-white uppercase">{formatMonthLabel(selectedMonth)}</strong> ({selectedMonth})</span>
          </div>
        ...
        {/* High-Impact Executive Summary Box (As Specifically Requested) */}
        <div className="p-6 bg-gradient-to-r from-zinc-900 via-zinc-900/95 to-zinc-950 border border-emerald-500/40 rounded-2xl shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
            ...
            <span className="text-[11px] text-zinc-400 font-mono">
              {formatMonthLabel(selectedMonth)} Summary
            </span>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono">
            <div className="p-3.5 bg-zinc-950/70 border border-zinc-800 rounded-xl space-y-1">
              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider block">ONLINE PAYMENT</span>
              <span className="text-xl sm:text-2xl font-black text-emerald-400 block">
                ₹{(summary.onlineSales || 0).toLocaleString('en-IN')}
              </span>
              <span className="text-[10px] text-zinc-500 font-sans block">({summary.onlineOrdersCount || 0} orders)</span>
            </div>

            <div className="p-3.5 bg-zinc-950/70 border border-zinc-800 rounded-xl space-y-1">
              <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider block">CASH/COD PAYMENT</span>
              <span className="text-xl sm:text-2xl font-black text-blue-400 block">
                ₹{(summary.codSales || 0).toLocaleString('en-IN')}
              </span>
              <span className="text-[10px] text-zinc-500 font-sans block">({summary.codOrdersCount || 0} orders)</span>
            </div>

            <div className="p-3.5 bg-zinc-950/70 border border-amber-500/30 rounded-xl space-y-1">
              <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider block">TOTAL PAYMENT</span>
              <span className="text-xl sm:text-2xl font-black text-amber-400 block">
                ₹{(summary.totalSales || 0).toLocaleString('en-IN')}
              </span>
              <span className="text-[10px] text-zinc-500 font-sans block">Collected revenue</span>
            </div>

            <div className="p-3.5 bg-zinc-950/70 border border-zinc-800 rounded-xl space-y-1">
              <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-wider block">TOTAL ORDERS</span>
              <span className="text-xl sm:text-2xl font-black text-white block">
                {summary.totalOrders || 0}
              </span>
              <span className="text-[10px] text-zinc-500 font-sans block">Total DB count</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 sm:p-5 bg-zinc-900/90 border border-emerald-500/30 rounded-2xl space-y-2 shadow-lg">
            <div className="flex items-center justify-between text-zinc-400">
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-emerald-400">Month's Revenue</span>
              <DollarSign className="w-5 h-5 text-emerald-400" />
            </div>
            ...
            <p className="text-[11px] text-zinc-400">
              Net valid sales for {formatMonthLabel(selectedMonth)}
            </p>
          </div>

          <div className="p-4 sm:p-5 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-2 shadow-lg">
            <div className="flex items-center justify-between text-zinc-400">
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-zinc-300">Total Monthly Orders</span>
          ...
          <div className="p-4 sm:p-5 bg-zinc-900/90 border border-emerald-500/30 rounded-2xl space-y-2 shadow-lg">
            <div className="flex items-center justify-between text-zinc-400">
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-emerald-400">Online Sales Share</span>
            ...
            <p className="text-[11px] text-zinc-400">
              {summary.onlineOrdersCount || 0} online payment orders
            </p>
          </div>

          <div className="p-4 sm:p-5 bg-zinc-900/90 border border-blue-500/30 rounded-2xl space-y-2 shadow-lg">
            <div className="flex items-center justify-between text-zinc-400">
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-blue-400">COD Sales Share</span>
            ...
            <p className="text-[11px] text-zinc-400">
              {summary.codOrdersCount || 0} cash on delivery orders
            </p>
          </div>
        </div>
        ...
        <div className="bg-zinc-900/90 border border-zinc-800 rounded-2xl overflow-hidden shadow-xl">
          <div className="p-4 sm:p-5 border-b border-zinc-800 flex items-center justify-between">
            <h3 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Orders Placed in {formatMonthLabel(selectedMonth)} ({ordersList.length})</span>
            </h3>
            <span className="text-[11px] text-zinc-400 font-mono">Full monthly history</span>
          </div>

          {loading ? (
            <div className="p-12 text-center text-zinc-400 flex flex-col items-center gap-3">
              <RefreshCw className="w-6 h-6 animate-spin text-emerald-400" />
              <span className="text-xs font-semibold">Loading monthly orders from database...</span>
            </div>
          ) : ordersList.length === 0 ? (
            <div className="p-12 text-center text-zinc-500 space-y-2">
              <Calendar className="w-8 h-8 text-zinc-600 mx-auto" />
              <p className="text-sm font-semibold text-zinc-400">No orders found for {formatMonthLabel(selectedMonth)}.</p>
              <p className="text-xs text-zinc-500">Try selecting a different month from the month selector above.</p>
            </div>
          ) : (
